...
function drawGraph(dot_info, attack_info, color_data, attack_vector_data ){    
    ...
    $(av_legend_obj).html("");
    $(header_row_obj).html('');
    $(left_total_legend_obj).html('<div class="data-row"><div class="data-col">Index</div></div>');
    ...
    for(var i = 0; i < dot_info.length; i++ ){
        ...
        if (dot_attacks ){
            ...
            $("<div>").addClass("data-col " + dot_item["ID"]).text(dot_item["DOT_Number"]).appendTo(data_row);
                ...
                $("<span>").addClass("circle " + has_class).attr("data-index-val", total )
                    ...
                    .on("click", function(e){
                        if($(this).hasClass("rec-val")){
                            ...
                            var url = getDomain() + "/bluewire/customer-portal/#Page:Recommendation_Engine_S?loginUserEmail=" + g_email + "&selectedDOTid=" + tmp_dotid + "&selectedAttackVectorID=" + tmp_attackid;
            ...
            var data_row = $("<div>").addClass("data-row")
                .appendTo(left_total_legend_obj);
            $("<div>").addClass("data-col total-" + dot_item["ID"]).text(0)
                .appendTo(data_row);
    ...
    // set the height
    var av_legneds = $(".av-legend");
    var max_width = 0;
    for (var i = 0; i < av_legneds.length; i++){
        if ($(av_legneds[i]).width() > max_width ){
            max_width = $(av_legneds[i]).width();
        }
    }

    $(".av-legend-container").css("min-height", (max_width + 100) + "px");
}
